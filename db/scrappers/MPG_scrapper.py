...
KBB_mpg = pd.DataFrame(
    columns=['Make', 'Model', 'Year', 'Combined Fuel Economy'])
        ...
        for year in this_car_years:
            if len(year) == 4:
                try:
                   ...
                   locate_mpg = texts.index("Combined Fuel Economy ") + 1
                   KBB_mpg = KBB_mpg.append({'Make': make, 'Model': model, 'Year': year, 'Combined Fuel Economy': texts[locate_mpg][:-4] }, ignore_index=True)
                   print(texts[locate_mpg][:-4] + ' is the KBB MPG for ' + make + ' ' + model + ' ' + year)
                except ValueError:
...
export_csv = KBB_mpg.to_csv (r'KBB_mpg.csv', index = None, header=True)
